    ...
    >
      ...
      <div className="relative overflow-hidden" style={{ aspectRatio: '4/3', background: '#f4f4f0' }}>
        {imageUrl ? (
          ...
          <div className="flex h-full items-center justify-center" style={{ background: 'linear-gradient(135deg, #eef3f8, #d4e1ef)' }}>
            <svg className="h-12 w-12" style={{ color: '#7aa2c8' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <div className="absolute inset-x-0 bottom-0" style={{ height: '4rem', background: 'linear-gradient(to top, rgba(10,15,29,0.45), transparent)' }} />

        {/* Badge type */}
        <div className="absolute left-3 top-3">
          <span className="inline-block px-2.5 py-1 text-xs font-semibold tracking-wide rounded"
            style={listing.type === 'vente'
        ...
        {listing.price_per_m2 && listing.type === 'vente' && (
          <div className="absolute bottom-3 right-3">
            <span className="inline-block px-2 py-1 text-xs font-medium rounded"
              style={{ background: 'rgba(10,15,29,0.75)', color: '#F59E0B', backdropFilter: 'blur(8px)', border: '1px solid rgba(212,175,55,0.2)' }}>
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="mb-2 text-xl font-bold" style={{ color: '#0D1B3E', letterSpacing: '-0.02em' }}>
          {priceLabel}
        </div>
        <h3 className="mb-4 line-clamp-2 text-sm leading-snug" style={{ color: '#374151', fontWeight: 500 }}>
          {listing.title}
        </h3>
        <div className="mt-auto flex items-center gap-4 text-xs pt-4" style={{ borderTop: '1px solid #e8e8e2', color: '#9ca3af' }}>
          <span className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="capitalize truncate font-medium" style={{ color: '#6b7280' }}>{location}</span>
          </span>
          {listing.surface_m2 && (
          ...
          <span className="ml-auto text-xs uppercase tracking-wider shrink-0" style={{ color: '#d1d5db', letterSpacing: '0.08em' }}>
